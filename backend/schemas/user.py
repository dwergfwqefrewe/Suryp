...
class UserOut(UserBase):
    """Схема для получения пользователя:
        ...
        - articles (List['ArticleResponse']) - список статей пользователя
        - followers (List['UserOut']) - список пользователей, которые подписаны на пользователя
        - following (List['UserOut']) - список пользователей, на которых подписан пользователь
    """
    id: int
    role: int
